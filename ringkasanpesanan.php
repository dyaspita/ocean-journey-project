...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Simpan ke database
    $query = "INSERT INTO pemesanan (nama_plg, telepon, alamat, destinasi, tgl_berangkat, jumlah_orang, metode_bayar)
              VALUES ('$nama_plg', '$telepon', '$alamat', '$destinasi', '$tgl_berangkat', '$jumlah_orang', '$metode_bayar')";
  ...
  <section class="py-20 px-6">
    <div class="max-w-6xl mx-auto bg-white rounded-2xl shadow-lg p-8 md:p-12 flex flex-col md:flex-row items-center gap-20">
      ...
      <div class="w-full md:w-1/2">
        ...
        <div>
             <h2 class="text-xl md:text-2xl font-bold text-[#a78d60]">Ringkasan Pesanan</h2>
             <br>
        </div>
        <hr class="mb-4 border-[#a78d60]">
        <div class="text-[#a78d60] space-y-1">
          <p><span class="font-semibold">No. Pesanan :</span> <?= htmlspecialchars($id_pesan) ?></p>
          <p><span class="font-semibold">No. Pesanan       :</span> <?= htmlspecialchars($id_pesan) ?></p>
          <p><span class="font-semibold">Nama Pemesan      :</span> <?= htmlspecialchars($nama_plg) ?></p>
          <p><span class="font-semibold">Alamat Domisili   :</span> <?= htmlspecialchars($alamat) ?></p>
          <p><span class="font-semibold">Nomor Telepon     :</span> <?= htmlspecialchars($telepon) ?></p>
          <p><span class="font-semibold">Pilihan Paket     :</span> <?= htmlspecialchars($destinasi) ?></p>
          <p><span class="font-semibold">Tanggal           :</span> <?= htmlspecialchars($tgl_berangkat) ?></p>
          <p><span class="font-semibold">Jumlah Orang      :</span> <?= htmlspecialchars($jumlah_orang) ?></p>
          <p><span class="font-semibold">Metode Pembayaran :</span> <?= htmlspecialchars($metode_bayar) ?></p>
        </div>
        <div class="mt-6">
